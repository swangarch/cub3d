...
int on_obj(t_vars *vars, t_vector* center, t_vector *intersect, int index)
{
    double x = intersect->x;
    double y = intersect->y;
    double top_y = center->y + GRID_SIZE / 2.0;
    ...
    if (double_eql(x, left_x) && y >= bottom_y + MIN_ERR2 && y <= top_y - MIN_ERR2)
    {
        vars->ray_obj_color[index] = WEST;
        vars->ray_obj_pos[index] = top_y - y;
        return (WEST);
    }
    else if (double_eql(x, right_x) && y >= bottom_y + MIN_ERR2 && y <= top_y - MIN_ERR2)
    {
        vars->ray_obj_color[index] = EAST;
        vars->ray_obj_pos[index] = y - bottom_y;
        return (EAST);
    }
    else if (double_eql(y, top_y) && x >= left_x + MIN_ERR2 && x <= right_x - MIN_ERR2)
    {
        vars->ray_obj_color[index] = NORTH;
        vars->ray_obj_pos[index] = right_x - x;
        return (NORTH);
    }
    else if (double_eql(y, bottom_y) && x >= left_x + MIN_ERR2 && x <= right_x - MIN_ERR2)
    {
        vars->ray_obj_color[index] = SOUTH;
        vars->ray_obj_pos[index] = x - left_x;
        return (SOUTH);
    }
    return (NOT_ONBOX);
}

double hit_obj(t_vars *vars, t_vector *intersect, int index, t_vector map_loc)
{
    // int map_x = (int)floor(intersect->x); // 当前网格单元 x 坐标
    // int map_y = (int)floor(intersect->y); // 当前网格单元 y 坐标
    t_vector box_pos;
    ...
    if (vars->map[(int)map_loc.y][(int)map_loc.x] == '1')
    {
        ...
        // 检查光线是否在墙壁的范围内
        if (on_obj(vars, &box_pos, intersect, index) != NOT_ONBOX)
            return (0);
    }
    if (vars->map[(int)map_loc.y][(int)map_loc.x] == 'C')
    {
        // 计算当前网格的中心坐标
        box_pos.x = map_loc.x + GRID_SIZE / 2.0;
        box_pos.y = map_loc.y + GRID_SIZE / 2.0;
...
double obj_distance(t_vars *vars, t_vector *vector, int index)
{
    ...
    while (1) {
        ...
        // 可根据需要在此处添加条件判断（例如检测碰撞或其他逻辑）
        distance_toobj = hit_obj(vars, &intersect, index, map_loc);
        if (distance_toobj >= 0)
            return (distance_toobj);
        if (len_2pt(&(vars->posv), &intersect) > MAX_VIEW_DISTANCE) 
            return(MAX_VIEW_DISTANCE);
    }
    return 0;
