...
double hit_wall(t_vars *vars, t_vector *intersect, int i, t_vector map_loc)
{
    t_vector box_pos;
    double distance;
    ...
    if (vars->map[(int)map_loc.y][(int)map_loc.x] == '1')
    {
        ...
        if (on_box(vars, &box_pos, intersect, i) != NOT_ONBOX)
        {
...
static void first_pos(t_vars *vars, t_vector *r, t_vector *v, t_vector *ist)
{
    r->x = floor(v->x);
    r->y = floor(v->y);
...
static void move_timeval(t_vector *intersect, t_vector *posv, double timeval,\
    t_vector *dv)
{
