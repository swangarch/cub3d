...
int on_box(double x_center, double y_center, double x, double y)
{
    ...
    if (double_eql(x, left_x) && y >= bottom_y + MIN_ERR && y <= top_y - MIN_ERR)
            return (WEST);
    else if (double_eql(x, right_x) && y >= bottom_y + MIN_ERR && y <= top_y - MIN_ERR)
            return (EAST);
    else if (double_eql(y, top_y) && x >= left_x + MIN_ERR && x <= right_x - MIN_ERR)
            return (NORTH);
    else if (double_eql(y, bottom_y) && x >= left_x + MIN_ERR && x <= right_x - MIN_ERR)
            return (SOUTH);
    return (NOT_ONBOX);
}
